    ...
    build: function () {
        ...
        if (this.format) {
            ...
            // Range Input
                ...
                //Brad's range output code                    
                var eventName = 'input'; // use onchange for select events, onclick for checkbox
                this.input.addEventListener(eventName, function (e) {
                    e.preventDefault();
                    e.stopPropagation();
                    
                    //Dirty way of stripping old values (regex numerical stripper)
                    //Currently has a bug - adding whitespace
                    str = self.input.nextElementSibling.textContent.replace(/[0-9]/g, '');
                    self.input.nextElementSibling.textContent = str + " " + self.input.value;
                });
            }
            // Source Code
            else if ([
